...
public:
  OpenCLAllocator(cl::Context &context, cl::CommandQueue &queue,
                  bool isInputBuffer)
...
public:
  ...
  OpenCLDepthPacketProcessorImpl(const int deviceId = -1)
    ...
    input_buffer_allocator =
        new PoolAllocator(new OpenCLAllocator(context, queue, true));
    ir_buffer_allocator =
        new PoolAllocator(new OpenCLAllocator(context, queue, false));
...
#pragma GCC diagnostic pop
  ...
  bool initBuffers() {
    ...
    CHECK_CL_PARAM(buf_filtered = cl::Buffer(context, CL_MEM_WRITE_ONLY,
                                             buf_filtered_size, NULL, &err));
  ...
  bool initProgram() {
    ...
    CHECK_CL_PARAM(kernel_processPixelStage2 =
                       cl::Kernel(program, "processPixelStage2", &err));
    ...
    CHECK_CL_PARAM(kernel_filterPixelStage2 =
                       cl::Kernel(program, "filterPixelStage2", &err));
  ...
  bool run(const DepthPacket &packet) {
    ...
    CHECK_CL_RETURN(queue.enqueueNDRangeKernel(
        kernel_processPixelStage2, cl::NullRange, cl::NDRange(IMAGE_SIZE),
        cl::NullRange, &eventFPS1, &eventPPS2[0]));

    if (config.EnableEdgeAwareFilter) {
      CHECK_CL_RETURN(queue.enqueueNDRangeKernel(
          kernel_filterPixelStage2, cl::NullRange, cl::NDRange(IMAGE_SIZE),
          cl::NullRange, &eventPPS2, &eventFPS2[0]));
    } else {
      eventFPS2[0] = eventPPS2[0];
    }

    CHECK_CL_RETURN(queue.enqueueReadBuffer(
        config.EnableEdgeAwareFilter ? buf_filtered : buf_depth, CL_FALSE, 0,
        buf_depth_size, depth_frame->data, &eventFPS2, &eventReadDepth));
  ...
  void newIrFrame() {
    ir_frame = new OpenCLFrame(static_cast<OpenCLBuffer *>(
        ir_buffer_allocator->allocate(IMAGE_SIZE * sizeof(cl_float))));
    ir_frame->format = Frame::Float;
  }

  void newDepthFrame() {
    depth_frame = new OpenCLFrame(static_cast<OpenCLBuffer *>(
...
void OpenCLDepthPacketProcessor::setConfiguration(
...
void OpenCLDepthPacketProcessor::loadP0TablesFromCommandResponse(
...
void OpenCLDepthPacketProcessor::loadXZTables(const float *xtable,
                                              const float *ztable) {
